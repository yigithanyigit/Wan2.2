            ...
            with (
                ...
                for i, t in enumerate(tqdm(timesteps)):
                    ...
                    noise_pred_cond = TensorList(
                         self.noise_model(TensorList(latent_model_input), t=timestep, **arg_c, cache=cache(step=i, cond_index=0))
                    )

                    if guide_scale > 1:
                        noise_pred_uncond = TensorList(
                             self.noise_model(
                                TensorList(latent_model_input), t=timestep, **arg_null, cache=cache(step=i, cond_index=1)
                            )
                        )
